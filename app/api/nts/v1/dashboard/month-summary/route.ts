    ...
    try {
        ...
        const [
            ...
            TiffinEntry.aggregate([
                ...
                { $group: { _id: null, total: { $sum: "$total_qty" }, amount: { $sum: "$total_amount" } } },
            ]),
            TiffinEntry.aggregate([
            ...
            Payment.aggregate([
                { $match: { payment_date: { $gte: thisMonthStart, $lt: nextMonthStart } } },
                { $group: { _id: null, amount: { $sum: "$paid_amount" } } },
            ]),
            Payment.aggregate([
                { $match: { payment_date: { $gte: lastMonthStart, $lt: thisMonthStart } } },
                { $group: { _id: null, amount: { $sum: "$paid_amount" } } },
            ]),
            Expense.aggregate([
