...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const [customers, targetEntries, sourceEntries] = await Promise.all([
            ...
            TiffinEntry.find({ entry_date: { $gte: targetDate, $lt: targetNext } }).lean(),
            fromDate
                ? TiffinEntry.find({
                      entry_date: {
        ...
        const targetMap = new Map(targetEntries.map((e) => [String(e.customer_id), e]))
        const sourceMap = new Map(sourceEntries.map((e) => [String(e.customer_id), e]))
        ...
        return success(rows, 'Preview ready', { date, fromDate, count: rows.length })
    } catch (e) {
