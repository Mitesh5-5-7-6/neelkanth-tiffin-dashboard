...
import type { ParsedWorkbookData, ParsedWorkbookCell, ImportPreviewRow, ImportValidationIssue } from '@/types/import.type'
...
export function matchCustomersToWorkbook(
    ...
    customers: Customer[]
): { previewRows: ImportPreviewRow[]; issues: ImportValidationIssue[]; duplicateNames: number } {
    ...
    for (const row of workbook.rows) {
        for (const cell of row.cells) {
            ...
            const matchedCustomer = normalizedName ? normalizedLookup.get(normalizedName) : undefined
