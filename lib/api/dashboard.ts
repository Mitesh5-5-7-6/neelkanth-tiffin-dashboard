...
export function getCategoryColor(displayLabel: string): string {
    const entry = Object.values(CATEGORY_META).find((m) => m.label === displayLabel)
...
interface RawCategoryPoint { category: string; amount: number }
...
export const dashboardApi = {
    ...
    expenseCategories: async (from: string, to: string): Promise<ExpenseCategoryPoint[]> => {
        const raw = await get<RawCategoryPoint[]>(`expense-categories?${dateParams(from, to)}`)
        return raw.map((r) => {
