...
import { useRecentExpenses, type RecentExpense } from "@/hooks/useDashboard"
import { getCategoryColor } from "@/lib/api/dashboard"
import EmptyState from "@/components/dashboard/EmptyState"

const col = createColumnHelper<RecentExpense>()
...
const columns = [
    ...
    col.accessor("category", {
        header: "Category",
        cell: (info) => {
            const color = getCategoryColor(info.getValue())
            return (
                <span
    ...
    col.accessor("description", {
        header: "Description",
        cell: (info) => (
            <span className="text-sm text-foreground max-w-45 truncate block">{info.getValue()}</span>
        ),
    }),
    col.accessor("amount", {
        header: "Amount",
        cell: (info) => (
            <span className="text-sm font-semibold text-danger">₹{info.getValue().toLocaleString("en-IN")}</span>
        ),
    }),
    col.display({
        id: "actions",
    ...
    return (
        <div className="bg-card rounded-2xl border border-border overflow-hidden">
            <div className="px-5 py-4 border-b border-border">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Last 7 days spending</p>
            </div>
                            ...
                            <tr>
                                <td colSpan={5}>
                                    <EmptyState
                                        ...
                                        description="Failed to fetch expense data. It will retry automatically."
                                    />
                                </td>
                            </tr>
                        ) : data.length === 0 ? (
                            <tr>
                                <td colSpan={5}>
                                    <EmptyState
                                        ...
                                        description="No expense entries found for the selected period."
                                    />
                                </td>
                            </tr>
                        ) : (
